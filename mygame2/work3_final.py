...
ardo = serial.Serial('/dev/cu.usbmodem1411', 9600)
...
foo_list = []
for i in range (2):  #range (4) means there is four elements in an array which means four black floor will generated
    floor_startx = random.randrange(0+20, display_width - floor_width - 20)  #floor x location generated constrain
    floor_starty = random.randrange(400,800)  #floor y location generated constrain
...
for i in range (2):
    ...
    rfloor_starty = random.randrange(400,800)
...
def game_loop():
    ...
    while not gameExit: #when the game is not closed:
        ...
        pygame.draw.rect(gameDisplay, red, (selocax,selocay,selocaw,selocah))
        ...
        if x + 30 > selocax and x < selocax + selocaw and y + 45 > selocay and y < selocay + selocah:
            buttonsound2.play()  #button sound will be played/call-up the build-in function that I have already defined before
            arduino() #get into floor_intro() page loop    
...
def gameover():
    ...
    while not gameExit:
        ...
        button("try again",display_width/2-60,display_height/2-90,120,40,red,lightred,arduino)
...
def arduino():
    ...
    while not gameExit:
        ...
        textline = ardo.readline().decode('ascii')
        n = float(textline)
        print (textline)
        x = n
        ...
        for item in foo_list:
            ...
            if item[1] < 0:
                item[1] = random.randrange(display_height, display_height+550)  
                item[0] = random.randrange(0 + 20, display_width -20 - floor_width)  
                marks += 1
                buttonsound4.play()

         
            if y + 45 >= item[1] - 10 and y + 45 <= item[1] - 1:  
                if x > item[0] and x < item[0] + floor_width or x + 30 > item[0] and x + 30 < item[0] + floor_width: 
                    gameover()
        ...
        for ritem in rfoo_list:
            ritem[1] += rfloor_speed
            pygame.draw.rect(gameDisplay, black, ritem)

            if ritem[1] < 0:
                ritem[1] = random.randrange(display_height, display_height + 400)
                ritem[0] = random.randrange(0 + 20, display_width -20 - rfloor_width)
                marks += 1
                buttonsound4.play()
